...
RakeGithub.define_repository_tasks(
  ...
  # Actions store only for now: dependabot-triggered test runs need the
  # Dependabot store too, which is provisioned org-wide by issue 19 (the
  # ambient gh token cannot write it). Until then dependabot PR tests fail
  # at the unlock step — accepted (see the cutover PR's deliberate-decisions
  # list).
  t.secrets = [
